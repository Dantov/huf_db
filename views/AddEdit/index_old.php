...
include('controllers/addEdit_Controller.php');
?>
<!DOCTYPE HTML>
<html>
	...
	<body class="<?=$_SESSION['assist']['bodyImg'];?>">
		...
		<div class="container">
				...
				<div class="col-sm-12">
					...
					<div class="panel panel-default">
						...
						<table class="table <?=$gs_len ? "" : "hidden"?>">
							...
							<tbody id="gems_table">
							<?php 
								for ( $i = 0; $i < $gs_len; $i++ ) { // автозаполнение если добавляем комплект или редакт модель
							?>
							<tr>
								<td><?=$i+1;?></td>
            ...
            <div class="row">
                ...
                <div class="col-xs-4">
                    <a type="button" class="btn btn-danger pull-right <?=$shownEdit;?> <?=$PDO_hide;?>" onclick="dell_fromServ(<?=$id;?>, false, false, 1);">
                        <span class="glyphicon glyphicon-remove"></span>
                        Удалить
                    </a>
                </div><!--end col-xs-6-->
			...
			<script src="../Main/js/main.js?ver=123"></script>
			<?=$stonesScript;?>
